...
function init(env, cb, forceNewConnection) {
  ...
  function maybe_connect(cb) {
    if (connection != null && !forceNewConnection) {
      ...
      if (!env.storageURI) {
        throw new Error(
          "MongoDB connection string is missing. Please set MONGO_CONNECTION environment variable"
        );
      }
      ...
      var connect_with_retry = function(i) {
        return MongoClient.connect(env.storageURI, options, function connected(
          err,
          client
        ) {
          if (err) {
  ...
  mongo.ensureIndexes = function ensureIndexes(collection, fields) {
    fields.forEach(function(field) {
      ...
      collection.createIndex(field, function(err) {
        if (err) {
          console.error("unable to createIndex for: " + field + " - " + err);
        }
      });
    });
  };
